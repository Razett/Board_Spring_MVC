...
<%@ include file="../include/header2.jsp" %>
                ...
                <div class="container-fluid">
                    ...
                    <div class="card shadow mb-4">
                        <div class="card-header py-3">
                            <h6 class="m-0 font-weight-bold text-primary">전체 게시물</h6>
                        </div>
                        <div class="card-body">
	                            	...
	                            	<div class="dataTables_paginate paging_simple_numbers" id="paginate">
	                            		<ul class="pagination mt-3">
	                            			<li class='paginate_button page-item previous ${pagenator.prev ? "" : "disabled"}' id="paginate_previous_li">
	                            				<a href="/board/list?pageNum=${pagenator.startPage - 1}" class="page-link" id="paginate_previous">Prev</a>
	                            			</li>
	                            				<c:forEach begin="${pagenator.startPage}" end="${pagenator.endPage}" var="num">
		                            				<li class='paginate_button page-item ${num == pagenator.criteria.pageNum ? "active" : ""}'>
		                            					<a href="/board/list?pageNum=${num}" class="page-link">${num}</a>
		                            				</li>
	                            				</c:forEach>
	                            			<li class='paginate_button page-item next ${pagenator.next ? "" : "disabled"}' id="paginate_next_li">
	                            				<a href="/board/list?pageNum=${pagenator.endPage + 1}" class="page-link" id="paginate_next">Next</a>
	                            			</li>
	                            		</ul>
	                            	</div>
	                            </div>
	...
	<script>
		...
		checkModal(registResult);
		
		history.replaceState({}, null, null);

		function checkModal(registResult) {
			if (registResult === '' || history.state) {
				// console.log(history.state);
				return;
			} else if(parseInt(registResult) <= 0) {
				$("#insertModalBody").text("게시물 등록에 실패하였습니다. 관리자에게 문의하십시오.");
				$("#insertModal").modal("show");
			} else if (parseInt(registResult) > 0) {
				$("#insertModalBody").text("${registedBno}번 글이 등록되었습니다.");
				$("#insertModal").modal("show");
			}
		}
	</script>
