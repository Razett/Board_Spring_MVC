...
<%@ include file="../include/sidebar_list.jsp" %>
<%@ include file="../include/header2.jsp" %>
                ...
                <div class="container-fluid">
                    ...
                    <div class="d-flex mb-2">
						<h1 class="h4 text-gray-800 px-2 my-auto">검색 결과</h1>
                    ...
                    <div class="card shadow mb-4">
                        <div class="card-header py-3">
                            <h6 class="m-0 font-weight-bold text-primary">"<c:out value='${pagenator.criteria.keyword}'/>" 을(를) 포함하는 검색 결과</h6>
                        </div>
                        <div class="card-body">
                            <div class="row mx-0">
                                <div class="ml-3" style="display: flex;flex-wrap: wrap;align-items: center;">
                                    <label class="my-0">검색 옵션</label>
                                </div>
                                <div class="col-sm-12 col-md-6">
                                        <label class="my-0">
                                            <select id="select_type" name="type" class="custom-select custom-select-sm form-control form-control-sm w-auto">
			                                	<option value="TCW" ${pagenator.criteria.type == "TCW" ? "selected" : ""}>전체</option>
			                                	<option value="T" ${pagenator.criteria.type == "T" ? "selected" : ""}>제목</option>
			                                	<option value="C" ${pagenator.criteria.type == "C" ? "selected" : ""}>내용</option>
			                                	<option value="W" ${pagenator.criteria.type == "W" ? "selected" : ""}>작성자</option>
			                                	<option value="TC" ${pagenator.criteria.type == "TC" ? "selected" : ""}>제목 및 내용</option>
			                      	  		</select>
                                        </label>
                                </div>
                            </div>
                            ...
                            <div class="table-responsive">
                                <table class="table table-borderless" id="dataTable2" width="100%" cellspacing="0">
                                    <tbody>
	                                    ...
	                                    <tr>
	                                    <td class="py-0">
	                                    <div class="mb-1 py-1" onclick="window.location.href='/board/get?origin=list&pageNum=${pagenator.criteria.pageNum}&bno=${board.bno}'">
	                            	...
	                            	<div class="dataTables_paginate paging_simple_numbers" id="paginate">
	                            		<ul class="pagination mt-3">
	                            			<li class='paginate_button page-item previous ${pagenator.prev ? "" : "disabled"}' id="paginate_previous_li">
	                            				<a href="/board/search?keyword=${pagenator.criteria.keyword}&type=${pagenator.criteria.keyword}&pageNum=${pagenator.startPage - 1}" class="page-link" id="paginate_previous">Prev</a>
	                            			</li>
	                            				<c:forEach begin="${pagenator.startPage}" end="${pagenator.endPage}" var="num">
		                            				<li class='paginate_button page-item ${num == pagenator.criteria.pageNum ? "active" : ""}'>
		                            					<a href="/board/search?keyword=${pagenator.criteria.keyword}&type=${pagenator.criteria.keyword}&pageNum=${num}" class="page-link">${num}</a>
		                            				</li>
	                            				</c:forEach>
	                            			<li class='paginate_button page-item next ${pagenator.next ? "" : "disabled"}' id="paginate_next_li">
	                            				<a href="/board/search?keyword=${pagenator.criteria.keyword}&type=${pagenator.criteria.keyword}&pageNum=${pagenator.endPage + 1}" class="page-link" id="paginate_next">Next</a>
	                            			</li>
	                            		</ul>
	                            	</div>
	                            </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <button class="close" type="button" data-dismiss="modal" aria-label="닫기">
                        <span aria-hidden="true">×</span>
                    </button>
                </div>
                <div class="modal-body" id="insertModalBody"></div>
                <div class="modal-footer">
                    <button type="button" data-dismiss="modal" aria-label="확인" class="btn btn-primary" >확인</button>
                </div>
            </div>
        </div>
    </div>
	...
	$(document).ready(function(){
	    $("#select_type").change(function(){
	        ...
	        $("#input_type").val(selectedValue);
	        window.location.href = '/board/search?keyword=${pagenator.criteria.keyword}&type=' + selectedValue;
	    });
	});
	</script>
</body>
